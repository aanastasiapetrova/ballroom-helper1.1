...
def get_general_marks_list(spacing=1, numbers_list=[], dance='', competition_name='', group='', judges=[], gn=0):
    ...
    pdf.ln(8)
    pdf.cell(pdf.w-20, 10, f'СВОДНАЯ ТАБЛИЦА СУДЕЙСКИХ ОЦЕНОК', border=0, align='C')
    pdf.ln(20)
    pdf.cell(pdf.w-20, 10, f'Группа: {group}', border=0, align='L')
    pdf.ln(8)
    pdf.cell(pdf.w-20, 10, f'Танец: {dance}', border=0, align='L')
    pdf.ln(20)
    ...
    pdf.output(abspath(f"ballroom_helper/docs/general_marks_blanks/{gn}/{dance[0]}.pdf"))